...
| UC6  |Менеджер бэк-офиса, АБС, SMS-шлюз       |Обработка депозитной заявки                           | 1. Менеджер находит заявку из интернет-банка.                  |
                                                                                                         ...
                                                                                                         депозита.                                                      |
| UC7  |Гость, Сайт                             |Просмотр кредитных предложений (сайт)                 | 1. Гость открывает страницу «Кредиты».                         |
                                                                                                         2. Сайт получает ставку по типу клиента.                       |
                                                                                                         3. Гость видит предложение и общую информацию из Кредитного    |
                                                                                                         конвейера.                                                     |
| UC8  |Клиент, Интернет-банк                   |Просмотр кредитов и предодобренного оффера            | 1. Клиент авторизуется.                                        |
                                                                                                         2. Система показывает список кредитов и персональное           |
                                                                                                         предодобренное предложение.                                    |
| UC9  |Гость, Сайт, SMS-шлюз                   |Подача кредитной заявки (сайт)                        | 1. Гость заполняет Ф. И. О. и телефон (опц. паспорт).          |
...
| UC10 |Клиент, Интернет-банк, SMS-шлюз         |Подача кредитной заявки (интернет-банк)               | 1. Клиент выбирает кредит, счёт зачисления, вводит данные.     |
                                                                                                         2. Подтверждает операцию СМС-кодом.                            |
                                                                                                         3. Система отправляет заявку и запускает упрощённый            |
                                                                                                         рескоринг.                                                     |
| UC11 |Менеджер отделения, АБС                 |Продолжение / создание кредитной заявки в отделении   | 1. Клиент приходит в офис.                                     |
                                                                                                         2. Менеджер ищет одобренную онлайн-заявку или создаёт новую.   |
                                                                                                         3. Оформляет кредит в АБС.                                     |
| UC12 |Сервис уведомлений, Клиент, SMS-шлюз    |СМС-оповещения о статусе заявки                       | 1. Изменился статус заявки в системе.                          |
